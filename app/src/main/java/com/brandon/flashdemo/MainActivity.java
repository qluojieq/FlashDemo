...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Hide status bar and set full screen
            ...
            ImageReader.OnImageAvailableListener readerListener = new ImageReader.OnImageAvailableListener() {
                ...
                private void saveToGallery(byte[] bytes) throws IOException {
                    ...
                    if (uri != null) {
                        ...
                        // Also save to internal list for the app's internal thumbnail/viewer
                        // Note: For simplicity, we're still using file paths in photoPaths.
                        // Ideally, we should transition to using Uris everywhere.
                        String absolutePath = getPathFromUri(uri);
                        if (absolutePath != null) {
                            runOnUiThread(() -> {
                                photoPaths.add(absolutePath);
                                updateThumbnail(absolutePath);
                ...
                private String getPathFromUri(Uri uri) {
                    // This is a simplified way to get a path from a MediaStore Uri.
                    // Note: In modern Android, you should work with Uris directly.
                    return uri.toString(); 
                }
            };
    ...
    private void updateThumbnail(String pathOrUri) {
        try {
            ...
            if (pathOrUri.startsWith("content://")) {
                bitmap = MediaStore.Images.Media.getBitmap(this.getContentResolver(), Uri.parse(pathOrUri));
            } else {
                bitmap = BitmapFactory.decodeFile(pathOrUri);
            }
            
            // For simplicity, we won't rotate the thumbnail again if the JPEG was already rotated by Camera2
            // or if the Gallery handles it. But we'll keep the logic if needed.
